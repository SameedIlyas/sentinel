...
| 2026-05-18 | A1 | **A1 landed.** Two new enum columns + three provenance fields on `clinic_ai_tools` (`model_training_status`, `practice_opt_out_state`, `opt_out_verified_at`, `opt_out_verified_by_user_id`, `model_training_status_evidence`). Alembic migration 018 with round-trip-tested downgrade. Pydantic v2 schemas with admin-only validator for `verified` state. `clinic.tool.trains_on_data` alert with 30-day idempotency window. Monthly compliance PDF gains "Tools that train on your data" row. Dashboard banner + editor (admin-gated). Five locked i18n keys in en + es (Spanish overlays version-controlled but `resolveDict` locale-switching not yet wired). | A1 plan executed end-to-end. |
| 2026-05-18 | A1+R2 | **R2 merged into A1 at `9168885`.** Six i18n dict files conflict-resolved (training-status keys and projected-role keys both retained at the same insertion point). RBAC snapshot guard `EXPECTED_USER_SHA256` updated to LF-canonical bytes (commit `13d1028`) — R2's worktree captured a CRLF SHA on a `core.autocrlf=true` checkout. Combined verification: backend pytest 208/208, frontend vitest 131/131, tsc clean. | Tier-A merge train complete. |
| 2026-05-18 | A1+R2 (review pass) | **Four-agent review (code, python, typescript, healthcare); 1 CRITICAL + 5 HIGH applied as 6 fix commits (`5a67b33`..`2cabb1e`).** Healthcare-reviewer (plan gate) APPROVED — HEALTH-2/5/6/7 closed. Fixed: validator fall-open → fail-closed (CRIT); 422 detail format → structured pydantic `errors(include_url=False, include_context=False)`; ToolEditor consumes `useAuth().productRole` (removes A1's pre-R2 duplicate auth fetch); alert dispatch uses `with_for_update` row lock + `db.flush()` for SQLite test correctness; ClinicProductRole literals renamed to `'practice_owner' \| 'practice_staff'` (resolves UserRole.ADMIN string collision); A1-scoped `datetime.utcnow()` → `datetime.now(timezone.utc)` (deprecation warnings 547→530). | Quality bar raised before PR open. |
| 2026-05-18 | A1+R2 (deferred follow-ups) | **MEDIUM/LOW review items recorded as post-merge work.** Healthcare follow-ups: (a) native US-Hispanic reviewer to revise Spanish translations ("BAA por escrito" → "Acuerdo de Asociado Comercial (BAA)"; "exclusión" → "exclusión voluntaria"); (b) surface per-tool "verified by / verified at" sub-table in the monthly compliance PDF for §164.312(b) audit depth. Engineering follow-ups: switch `payload_raw: dict` → `Annotated[dict, Body()]` for OpenAPI sanity; type `ToolResponse` enum fields with the new enum classes instead of `str`; reference `ClinicAiToolModelTrainingStatus.TRAINS_ON_CUSTOMER_DATA` in `clinic_pdf_report.py` instead of the literal string; add a runtime-validated API-layer `ClinicAiTool` type (Zod) to replace the locally-typed `Tool` interface in `ToolList.tsx`; wire `resolveDict` for `es` locale; remove i18n triplication where the spread chain already provides inheritance; update snapshot-guard maintainer comment to recommend `git cat-file blob HEAD:path \| sha256sum` for capture (avoids the CRLF re-trap); add test for `trainingBanner.ts` `opt_out_available + not_applicable` branch. None of these block merge. | Logged so they're not lost. |
