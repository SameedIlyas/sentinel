...
import { dispatchMock, withDelay } from './mocks/router';
import { demoUser } from './mocks/fixtures';

const API_BASE_URL =
  import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000';

// When VITE_MOCK_API=true the client never touches the network. Every
// request is routed through ./mocks/router and answered with fixture
// data. Auth becomes a no-op that returns the demo admin user, so the
// dashboard auto-logs-in and works as a fully static SPA on Vercel.
...
class ApiClient {
  ...
  constructor() {
    ...
    this.client.interceptors.response.use(
      ...
      async (error: AxiosError) => {
        ...
        // Auth bootstrap endpoints must propagate 401 directly. Auto-
        // refreshing on `/v1/auth/refresh` would recurse; auto-refreshing
        // on `/v1/auth/validate` defeats the purpose (it IS the probe).
        // Login/logout 401s mean exactly what they say.
        const url = originalRequest?.url ?? '';
        const isAuthEndpoint =
          url.includes('/v1/auth/refresh') ||
          url.includes('/v1/auth/validate') ||
        ...
        if (
          ...
          try {
            ...
            this.clearAuth();
            // React Router will pick up the unauthenticated state on the
            // next render via AuthContext; avoid a full page reload here
            // so the bootstrap promise can settle cleanly.
            return Promise.reject(refreshError);
          }
        }

        if (error.response?.status === 401 && isAuthEndpoint) {
          // Bootstrap failed — clear any stale local state so AuthContext
          // settles into the unauthenticated branch.
          this.clearAuth();
        }
  ...
  isAuthenticated(): boolean {
    ...
    //
    // Mock mode always reports authenticated so the dashboard skips the
  ...
  async get<T>(url: string, params?: any): Promise<T> {
    if (MOCK_API) return withDelay(dispatchMock<T>('get', url, params));
    const response = await this.client.get<T>(url, { params });
    return response.data;
  }

  async post<T>(url: string, data?: any): Promise<T> {
    if (MOCK_API) return withDelay(dispatchMock<T>('post', url, undefined, data));
    const response = await this.client.post<T>(url, data);
    return response.data;
  }

  async put<T>(url: string, data?: any): Promise<T> {
    if (MOCK_API) return withDelay(dispatchMock<T>('put', url, undefined, data));
    const response = await this.client.put<T>(url, data);
    return response.data;
  }

  async delete<T>(url: string): Promise<T> {
    if (MOCK_API) return withDelay(dispatchMock<T>('delete', url));
    const response = await this.client.delete<T>(url);
    return response.data;
  }

  async patch<T>(url: string, data?: any): Promise<T> {
    if (MOCK_API) return withDelay(dispatchMock<T>('patch', url, undefined, data));
  ...
  async getBlob(url: string, params?: any): Promise<Blob> {
    if (MOCK_API) {
      const payload = dispatchMock('get', url, params);
      return withDelay(
